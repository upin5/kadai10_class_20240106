...
foreach ($makeCharacters as $makeCharacter){
    ...
    foreach ($ff7Characters as $ff7Character){
        ...
        $characterDifferences[$ff7Character['name']] = $totalDifference;
    }
...
// echo "初期の minTotalDifference: " . $minTotalDifference;
        ...
        $closestCharacterName = array_keys($characterDifferences, min($characterDifferences))[0];
        // var_dump($closestCharacterName);


        echo '<p style="color:white; font-size: 24px; text-shadow: 2px 2px 4px rgba(255, 255, 255, 0.5); background-color:rgba(0, 0, 0, 0.5);">あなたのキャラクタータイプ：' . $closestCharacterName . '</p>';
